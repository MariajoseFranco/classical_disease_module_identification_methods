...
def _get_path_to_study_bias_scores(study_bias_scores, namespace):
    if isinstance(study_bias_scores, pd.DataFrame):
        study_bias_scores.to_csv(f'./state_of_art_repos/robust_bias_aware/data/study_bias_scores/{namespace}/custom_study_bias_scores.csv', index=False)
        study_bias_scores=f'./state_of_art_repos/robust_bias_aware/data/study_bias_scores/{namespace}/custom_study_bias_scores.csv'
        ...
        if study_bias_scores not in ['None','BAIT_USAGE', 'STUDY_ATTENTION']:
            ...
            if study_bias_scores=='None':
                return study_bias_scores
    return f'./state_of_art_repos/robust_bias_aware/data/study_bias_scores/{namespace}/{study_bias_scores}.csv'
...
def _check_and_preprocess_network(network, namespace):
    ...
    if type(network) is nx.Graph:
        ...
        if network.endswith('.graphml'):
            ...
            network = f'./state_of_art_repos/robust_bias_aware/data/networks/{namespace}/{network}.txt'
        ...
        network.to_csv(f'./state_of_art_repos/robust_bias_aware/data/networks/{namespace}_customNetwork.txt', index=False, sep=' ')
        network=f'./state_of_art_repos/robust_bias_aware/data/networks/{namespace}_customNetwork.txt'
