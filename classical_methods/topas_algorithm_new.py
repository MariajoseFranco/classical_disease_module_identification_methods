...
class TOPAS():
    ...
    def _extract_lcc(self, graph, seeds):
        components = list(nx.connected_components(graph))
        max_seeds = 0
        best_component = None
        for comp in components:
            seed_count = len(seeds & comp)
            if seed_count > max_seeds:
                max_seeds = seed_count
                best_component = comp
        return graph.subgraph(best_component).copy()
    ...
    def _random_walk_prune(self, subgraph, seeds):
        ...
        for _, row in df[df["val"] == 0].iterrows():
            ...
            if nx.number_connected_components(temp_subgraph) > 1:
                # G_lcc = self.lcc.run_lcc_per_disease(temp_subgraph, seeds)
                lcc_temp = self._extract_lcc(temp_subgraph, seeds)
    ...
    def run(self, network_file, seeds_file):
        ...
        G_lcc = self._extract_lcc(G, seeds)
        # G_lcc = self.lcc.run_lcc_per_disease(G, seeds)
        ...
        subgraph = self._extract_lcc(subgraph, seeds)
        # subgraph = self.lcc.run_lcc_per_disease(subgraph, seeds)
